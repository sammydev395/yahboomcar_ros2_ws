  ...
  yahboomcar_msgs__msg__PointArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    yahboomcar_msgs__msg__PointArray * data =
      (yahboomcar_msgs__msg__PointArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yahboomcar_msgs__msg__PointArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          yahboomcar_msgs__msg__PointArray__fini(&output->data[i]);
        }
        return false;
      }
    }
